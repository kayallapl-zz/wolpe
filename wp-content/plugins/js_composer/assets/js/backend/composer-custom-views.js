...
(function ($) {
        ...
        render:function () {
            ...
            this.$content.sortable({
                ...
                stop:function (event, ui) {
                    ...
                    $(this).find('> .wpb_vc_accordion.wpb_sortable').each(function () {
        ...
        render:function () {
            ...
            this.$content.sortable({
                ...
                stop:function (event, ui) {
                    ...
                    // so trigger focusout handlers to remove .ui-state-focus
                    ui.item.prev().triggerHandler("focusout");
                    $(this).find('> .wpb_vc_nested_accordion.wpb_sortable').each(function () {
                        var shortcode = $(this).data('model');
                        shortcode.save({'order':$(this).index()}); // Optimize
                    });
                }
    ...
    window.VcNestedAccordionTabView = window.VcColumnView.extend({
		events:{
			'click > .vc_controls .column_delete,.wpb_vc_nested_accordion_tab > .vc_controls .vc_control-btn-delete':'deleteShortcode',
            'click > .vc_controls .column_add,.wpb_vc_nested_accordion_tab >  .vc_controls .vc_control-btn-prepend':'addElement',
            'click > .vc_controls .column_edit,.wpb_vc_nested_accordion_tab >  .vc_controls .vc_control-btn-edit':'editElement',
            'click > .vc_controls .column_clone,.wpb_vc_nested_accordion_tab > .vc_controls .vc_control-btn-clone':'clone',
            'click > [data-element_type] > .wpb_element_wrapper > .vc_empty-container':'addToEmpty'
        },
        setContent:function () {
            this.$content = this.$el.find('> [data-element_type] > .wpb_element_wrapper > .vc_container_for_children');
	...
	window.VcNestedTabsView = vc.shortcode_view.extend({
        ...
        cloneModel:function (model, parent_id, save_order) {
            ...
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'vc_nested_tab' ? false : true), cloned_from:model.toJSON(), params:new_params});
            _.each(Shortcodes.where({parent_id:model.id}), function (shortcode) {
                this.cloneModel(shortcode, model_clone.get('id'), true);
            }, this);
            return model_clone;
